...
from config.settings import ZAI_MODEL
...
def _prepare_zai_messages(
    ...
    into messages that are valid for Z.ai's
...
def ask_zai(
    ...
    GLM-4.7-Flash does not guarantee
    ...
    if not api_key:
        ...
        raise RuntimeError(
            ...
            "using the Z.ai provider."
        )

    messages = _prepare_zai_messages(
        conversation
    )
    ...
    if not choices:
        ...
        raise RuntimeError(
            "Z.ai returned no completion choices."
        )
    ...
    if not content:
        ...
        raise RuntimeError(
            "Z.ai returned an empty model response."
        )
